...
Base Vectorial (Clustering Asíncrono, Versionado y Aislado): Microservicios calculan en background el embedding inyectando el `tenant_id` y su `embedding_version` preventiva (Lazy Migration). Cada tenant tiene su propio punto Qdrant (`point_id = uuid5(recurso_id:tenant_id)`), pero el contenido del recurso es global: si dos usuarios suben la misma URL, el segundo reusa el vector ya calculado sin volver a llamar al embedder. Crucialmente, apenas guardan una idea inédita, buscan vectores hiper-compatibles inmediatos (Similitud del Coseno > 0.92). Si los hallan, cruzan esa asociación en la base relacional. Destruye los silos de información aislados: dos ideas en meses distintos colisionan en conocimiento útil autodescubierto, fomentando *Insights* y serendipias genuinas de altísimo valor cognitivo para el usuario.
...
1. Consideraciones Críticas de Infraestructura
...
* Aislamiento de Múltiples Inquilinos (Multi-Tenancy): Protección absoluta contra brechas de privacidad y filtración cruzada. Se exige como arquitectura base inyectar reglas estructurales sólidas: prefijos de clave en Redis (ej. `session:user_123:chat_456`), seguridad a nivel de fila (Row-Level Security) en la **tabla pivote `usuario_recursos`** (la `recursos` es global y deduplicada por `url_hash`) y el filtrado metadato subyacente `tenant_id` en Pinecone/Qdrant, allanando el camino para certificaciones empresariales como SOC2.
