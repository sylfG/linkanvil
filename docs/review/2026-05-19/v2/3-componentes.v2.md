...
**Origen**: `docs/src/3-componentes.md` · branch `develop` @ `7c723f3`
**Review aplicado**: `docs/review/2026-05-19/docs/3-componentes.review.md`

### Cambios aplicados
- 2 CRITICAL · 3 HIGH · 6 MEDIUM · 2 LOW (de un total de 15 hallazgos del review)
- Secciones tocadas:
  - Encabezado (§ intro): count 22 → 24 servicios; links internos `_` → `-` (10 ocurrencias).
  - §1 Traefik: rate limit precisado (`average=100/s`, `burst=50`, por IP).
  - §2 cerebro-ingestion: añadida nota sobre `clientes/browser-extension/` como cliente principal.
  - §2 cerebro-web: eliminado bullet "Diseño Antifragilidad (Error Boundaries)" sin evidencia; añadida nota sobre `src/ui/chatbot.py` legacy.
  - §3 intro Workers: heartbeat corregido a "cada 15s con TTL 45s" (antes "cada 45s").
  - §3 + Nueva ficha: **cerebro-notifier** (worker async que faltaba).
  - §3 cerebro-outbox: query sin prefijo `cerebro.` aclarada vía `search_path`; añadida cola `q.embeddings`.
  - §4 cerebro-litellm: añadidos números reales del circuit breaker (`allowed_fails: 1`, `cooldown_time: 15`), TTL caché (`3600`) y `store_model_in_db: true`.
  - §5 + Nueva ficha: **cerebro-migrate** (one-shot de migraciones SQL).
  - §5 cerebro-redis: precisada imagen `redis/redis-stack-server` con pinning por SHA256.
  - §5 cerebro-redis (bloque heartbeat): "cada 45 segundos" → "cada 15 segundos (con TTL de 45 s)".
  - Footer: links `2_resumen_servicios.md`, `4_arquitectura.md`, `1_instalacion_y_configuracion.md` reescritos a la forma con guion.

### Pendientes (no aplicados en este v2)
- **[UNVERIFIED] Trace-ID inyectado por Traefik** (línea 35 / línea 42 del v2): el review no pudo confirmar si Traefik 3.x propaga un header `X-Trace-Id` o solo emite spans OTLP. Requiere validación en runtime con `curl -v https://ingest.localhost/...` mirando los headers que llegan al backend. El texto actual del doc se mantiene; si se confirma que solo hay span OTLP sin header, reformular el bullet "Etiqueta de Seguimiento (Trace-ID)" según la sugerencia del review.
- **[UNVERIFIED] Bloom Filter "hint, no veto"** (línea 89 del original / equivalente en v2): el review no inspeccionó el call-site en `src/ingestion/main.py` con suficiente profundidad para confirmar si el `202 Accepted` se devuelve igualmente ante hit de Bloom o si se corta. El texto actual ("hint, no veto") se mantiene pendiente de verificación contra el call-site exacto.

### Bugs de código flaggeados (no son drift de doc, requieren acción aparte)
- Ninguno. El review no reportó hallazgos [CODE-BUG].
