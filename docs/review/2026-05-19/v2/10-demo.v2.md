...
2. Click en uno de los CTAs **"Probar el demo gratis"** (Hero) /
   **"Acceder al demo"** (CTABanner). Ambos están solo en la landing,
   no en el Nav. El Nav anónimo guarda su slot CTA para **"Iniciar
   sesión"** por decisión explícita de producto: ese botón es para
   quien ya viene a entrar a su cuenta, no para descubrir el demo.
...
(El backend FastAPI no aplica prefix `/api`; si tu deploy usa un
reverse-proxy que lo añada, ajusta la URL al lado del proxy.)
...
Antes del Slice 6.3, un visitante podía hacer logout y pulsar "Probar
el demo gratis" otra vez para conseguir 15 min fresquitos saltándose
la cuota diaria. Ahora `/auth/demo-start` consulta Redis
...
(FastAPI envuelve el dict pasado a `HTTPException` bajo la clave
`detail`; el front lee `err.detail.error` / `err.detail.message` /
`err.detail.redirect`.)
...
- El JWT lo emite `POST /auth/demo-start` con el claim
  `tenant_id = "demo_<8hex>"` (no el del usuario demo en BD
  `user_demo_landing`). `/auth/login` rechaza la cuenta demo con 403;
  `/auth/refresh` rota tokens manteniendo el `session_tenant`.
...
El loop pregunta primero a `db.get_sessions_with_due_events()`
(aprovecha el índice parcial `idx_demo_events_due` para barrer solo
eventos pending), y por cada tenant abre su propia transacción +
`set_config(app.tenant_id, ...)` + llama a
`run_demo_audit_for_session`. Las transacciones aisladas garantizan
...
en los 5s siguientes — el visitante ve el punto cambiar de su icono
"pending" (depende del kind: `AlertTriangle` para cuarentena,
`CalendarX` para archivo, `Hourglass` para reminder) al icono
"disparado" (`CheckCircle2` para todos) en vivo.
...
El background task `_cleanup_demo_sessions_loop` (cada 60 s) detecta
sesiones con `expires_at < NOW()`. Por cada una hace:
...
2. **Cascade transaccional en BD** (delegada a
   `delete_demo_session_cascade`):
   1. `SELECT set_config('app.tenant_id', tenant_id, true)` para
      satisfacer la RLS forced sobre `usuario_recursos` /
      `sesiones_chat`.
   ...
   6. `DELETE FROM demo_sessions` — dispara `ON DELETE CASCADE` sobre
      `demo_session_events` (Slice 6, migración 0010), así que los 4
      eventos programados desaparecen solos. Cero limpieza manual de
      la tabla de eventos.
...
│                                                                │
│   Landing CTA (Hero / CTABanner) → POST /auth/demo-start       │
│       ↓                                                        │
│   create_demo_session():                                       │
...
│   + gate "1 sesión / IP / día UTC" (Slice 6.3, Redis)          │
│       ↓                                                        │
...
- **`demo_already_used_today`** (Slice 6.3): respuesta 429 del
  endpoint `/auth/demo-start` cuando la IP ya gastó su sesión del día
  UTC. Incluye `register_url: /register`.
...
- Endpoints:
  - `POST /auth/demo-start` (Slice 6 — entrada sin password; emite el JWT con `tenant_id = demo_<8hex>`)
  - `POST /auth/login` (rechaza la cuenta demo con 403 `demo_use_dedicated_endpoint`)
  - `POST /auth/refresh` (rota tokens manteniendo el `session_tenant`)
...
- CTAs de la landing: `(marketing)/_components/{Hero,CTABanner}.tsx`
  (el `Nav.tsx` NO tiene CTA demo por decisión explícita — solo
  "Iniciar sesión" para anónimos)
...
- FK CASCADE `mensajes_chat → sesiones_chat`: ver migración baseline
...
**Origen**: `/tmp/linkanvil-mirror/docs/src/10-demo.md` · branch `develop` @ `7c723f3`
**Review aplicado**: `/tmp/linkanvil-staging/docs/review/2026-05-19/docs/10-demo.review.md`

### Cambios aplicados

- **2 CRITICAL · 3 HIGH · 4 MEDIUM · 1 LOW** (de un total de 11 hallazgos del review).
- Secciones tocadas:
  - §"Para el visitante" (línea 44-52): CTAs corregidos — solo Hero +
    CTABanner, Nav no duplica; literales reales `"Probar el demo
    gratis"` / `"Acceder al demo"`; redirect descrito como decisión
    del cliente (`router.push`), no del backend; eliminada la mención
    falsa a "tabs" en `/demo`.
  - §"Para el desarrollador / integraciones" (línea 55-64): URL del
    endpoint sin prefix `/api`; respuesta incluye `resumed` y
    `seconds_remaining?` (Slice 6.3).
  - §"Una sesión demo por IP por día UTC" (nueva): documenta gate
    Redis `demo_session_started:{ip}:{YYYY-MM-DD}` (Slice 6.3).
  - §"`/login` rechaza el email demo": estructura JSON corregida
    (cuerpo bajo clave `detail`, comportamiento estándar de
    `HTTPException` con dict).
  - §"En código" (líneas 99-104 originales): JWT emitido por
    `/auth/demo-start`, no por `/auth/login`; mención de
    `/auth/refresh`.
  - §"Quién dispara el audit": añadida descripción de
    `get_sessions_with_due_events` + transacciones aisladas por
    tenant.
  - §"La vista `/demo` — timeline en vivo": icono pending depende
    del `kind` (AlertTriangle / CalendarX / Hourglass), no siempre
    AlertTriangle.
  - §"Lo que se borra al expirar": cascada reescrita con el orden y
    detalles reales de `delete_demo_session_cascade`
    (`set_config(app.tenant_id)`, salvaguarda doble `NOT EXISTS`,
    Qdrant es responsabilidad del caller).
  - §"Diferencias con un usuario registrado": añadido el
    "1 sesión/IP/día UTC" a la fila de Cuota diaria.
  - §"Mapa mental rápido": label del CTA (Hero / CTABanner) +
    mención del gate de Slice 6.3.
  - §"Glosario rápido": añadida entrada `demo_already_used_today`.
  - §"Para investigar más": Backend amplía endpoints (`/auth/login`,
    `/auth/refresh`); Frontend aclara que `Nav.tsx` no tiene CTA
    demo; Datos añade referencia a la migración baseline para la FK
    CASCADE de `mensajes_chat`.

### Pendientes (no aplicados en este v2)

- **LOW omitido (LOW#2 sobre FK CASCADE de `mensajes_chat`)**: la
  referencia se añadió igualmente como ítem en "Para investigar más"
  > Datos. No es un cambio narrativo, solo enlace.
- UNVERIFIED del review: ninguno (el review reportó 0).
- TODOs opcionales: ninguno.

### Bugs de código flaggeados (no son drift de doc, requieren acción aparte)

- Ninguno — el review reportó 0 `[CODE-BUG]`.
