...
- "summary": resumen de 2-3 frases en español. Si el contenido es documentación versionada (URL con /v\d+/, /N.x/, /api/N/, etc.) o se refiere explícitamente a una versión concreta de un producto/framework/spec/norma, MENCIONA en el resumen: (1) el número de versión del documento (p.ej. 'Laravel 12.x', 'Python 3.11', 'RFC 9110', 'Real Decreto 686/2010'); (2) si el texto referencia una versión POSTERIOR que la sustituye/actualiza/obsoleta ('Obsoletes RFC X', 'Superseded by Y', 'derogado por Real Decreto Z', 'deprecated in favor of W'), nómbrala; y (3) si la página lista fechas de EOL/soporte/security-fixes, incluye la fecha clave aplicable a esta versión. Si no detectas versionado explícito, resume normalmente
