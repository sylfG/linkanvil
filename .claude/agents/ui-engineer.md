---
name: ui-engineer
description: "UI/UX specialist for both building new components and improving existing ones. Detects mode automatically: BUILD (feature description → new component from scratch) or REFACTOR (existing code → apply design rules). Applies 10 design priority categories: accessibility → touch → performance → style → layout → typography → animation → forms → navigation → charts."
