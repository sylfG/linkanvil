...
**Origen**: `docs/src/0-resumen.md` · branch `develop` @ `7c723f3`
**Review aplicado**: `docs/review/2026-05-19/docs/0-resumen.review.md`

### Cambios aplicados

- 0 CRITICAL · 1 HIGH · 3 MEDIUM · 2 LOW (de un total de 9 hallazgos del review)
- Secciones tocadas:
  - §"Estructura y Módulos del Sistema" (línea 38): reformulada como "22 servicios desplegados en Docker Compose alrededor de cinco grandes capas funcionales" con link a `3-componentes.md` [MEDIUM].
  - §A "Verificación de Duplicados" (líneas 45–48): sub-bullets re-indentados para que el render Markdown los anide correctamente bajo el ítem 2 [LOW].
  - §B "Formato de Datos Estricto" (línea 60): typo "estan" → "están" [LOW].
  - §E "Interfaz y Chatbot": ampliada con las tres superficies de UI reales (Next.js / `cerebro-web`, Streamlit / `src/ui/chatbot.py`, browser extension / `clientes/browser-extension/`) verificadas vía `ls /tmp/linkanvil-mirror/src/ui/chatbot.py` [MEDIUM].
  - §E "Chat Fluido": añadida mención al canal SSE reactivo que refleja en directo el estado de los enlaces (procesado / cuarentena / expirado) [MEDIUM].
  - §E "Seguridad de Acceso": claim genérico "cookies ultra-protegidas que los virus no pueden leer" reemplazado por descripción técnica precisa (`httpOnly`, `SameSite` configurable, 5 intentos de login/min/IP).
  - §"Infraestructura — Control de Consumo de Memoria" (línea 135): **eliminado** el claim sobre archivado de sesiones de chat tras 30 días de inactividad (no implementado en el código) y reemplazado por "Cuarentena de Recursos Obsoletos" que sí está implementado en `src/data/audit_cron.py:238` y `src/data/db.py:509` (`quarantine_grace_until = NOW() + INTERVAL '30 days'`) [HIGH].

### Pendientes (no aplicados en este v2)

- **[UNVERIFIED] §C "puente de conocimiento automático" entre recursos semánticamente similares (línea 69)**: existe `grafo_relaciones` y mecánica de colisión semántica (`src/data/embedder_worker.py:345`, `src/data/db.py:443`), pero confirmar que la inserción automática se dispara sin intervención del usuario requiere un walkthrough del embedder en runtime. Se preserva la redacción original a la espera de validación adicional.
- **[UNVERIFIED] §B "Conexión Inteligente y de Respaldo" → fallback OpenAI/secundario (línea 56)**: LiteLLM permite fallbacks declarativos en `infra/litellm/`, pero la política efectiva depende del `config.yaml` en runtime y no es auditable estáticamente. Se preserva la redacción original; pendiente validar contra `infra/litellm/config.yaml`.

### Bugs de código flaggeados (no son drift de doc, requieren acción aparte)

- Ninguno. El review no identificó hallazgos `[CODE-BUG]`.
