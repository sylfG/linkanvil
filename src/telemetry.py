...
def configure_telemetry(service_name: str) -> bool:
    ...
    try:
        ...
        logger.info(
            f"OpenTelemetry configurado exitosamente para el servicio: {service_name} apuntando a {OTEL_COLLECTOR_URL}"
        )
        return True
    except ImportError as e:
        logger.warning(
            f"Librerías de OpenTelemetry no instaladas. Fallback a trazabilidad logs: {e}"
        )
        return False
    except Exception as e:
        logger.error(
            f"Fallo crítico al configurar OpenTelemetry (Fallback Edge Case): {e}"
        )
        return False
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            with tracer.start_as_current_span(op_name) as span:
                trace_id = (
                    kwargs.get("trace_id") or kwargs.get("tr_id") or str(uuid.uuid4())
                )
