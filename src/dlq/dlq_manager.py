...
class DLQManager:
    ...
    async def retry_dead_letters(
        self, dlq_name: str, target_routing_key: str, target_exchange: str = ""
    ):
        ...
        queue = await self.channel.get_queue(dlq_name, ensure=False)
        exchange = (
            await self.channel.get_exchange(target_exchange)
            if target_exchange
        ...
        while True:
            try:
                ...
                new_msg = aio_pika.Message(
                    body=msg.body, content_type=msg.content_type, headers=msg.headers
                )
        ...
        logger.info(
            f"Re-encolados {requeued} mensajes desde la DLQ {dlq_name} hacia {target_routing_key}."
        )
        return requeued
