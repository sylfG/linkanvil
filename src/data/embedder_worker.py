...
def _qdrant_chunk_point_id(recurso_id: str, tenant_id: str, chunk_idx: int) -> str:
    return str(
        uuid.uuid5(_QDRANT_POINT_NS, f"{recurso_id}:{tenant_id}:chunk:{chunk_idx}")
    )


def _chunk_text(
    text: str, target_chars: int = 1200, overlap_chars: int = 150
) -> list[str]:
    ...
    if not paragraphs:
        ...
        return [
            ...
            if text[i : i + target_chars].strip()
        ]
...
class EmbedderWorker:
    ...
    async def _inject_chunks_to_qdrant(
        ...
        if not chunks or len(chunks) != len(vectors):
            logger.warning(
                f"[{trace_id}] inject_chunks: nada que upsert (chunks={len(chunks)} vectors={len(vectors)})"
            )
            return
        points = []
        ...
        resp.raise_for_status()
        logger.info(
            f"[{trace_id}] {len(points)} chunks inyectados en cerebro_chunks (recurso={recurso_id} tenant={tenant_id})"
        )
    ...
    async def _fetch_existing_vector_for_recurso(
        self, recurso_id: str
    ) -> Optional[dict]:
    ...
    async def _classify_collision_type(
        self, trace_id: str, tenant_id: str, new_info: dict, old_id: str
    ) -> str:
    ...
    async def _compute_semantic_collisions(
        self,
        recurso_id: str,
        tenant_id: str,
        vector: list[float],
        ...
        if resp.status_code != 200:
            logger.error(
                f"[{trace_id}] Error consultando Qdrant para similitud: {resp.text}"
            )
            return
        ...
        for r in results:
            ...
            tipo_relacion = await self._classify_collision_type(
                trace_id, tenant_id, extracted_info, other_recurso_id
            )
        ...
        if not collisions:
            logger.info(
                f"[{trace_id}] Sin colisiones semánticas > 0.92 para {recurso_id}"
            )
            return

        logger.info(
            f"[{trace_id}] Encontradas {len(collisions)} colisiones semánticas para {recurso_id}"
        )
        ...
        async with message.process(requeue=False, ignore_processed=True):
            ...
            trace_id = payload.get("trace_id") or (
                "-".join(
                    ...
                    .get("traceparent", "00-unknown-00-00")
                    .split("-")[1:3]
                )
                if "traceparent" in (message.headers or {})
                else (
                    message.headers.get("trace_id", "unknown-trace")
                    if message.headers
                    else "unknown-trace"
                )
            )
            ...
            try:
                if reused:
                    ...
                    if not src or not src.get("vector"):
                        ...
                        ext_info = (
                            await self._fetch_recurso_ext_info(recurso_id) or ext_info
                        )
                    ...
                    await self._inject_to_qdrant(
                        recurso_id, tenant_id, vector, ext_info, url, trace_id
                    )
                    await self._compute_semantic_collisions(
                        recurso_id, tenant_id, vector, ext_info, trace_id
                    )
                    ...
                    auto_archive = await self._fetch_auto_archive_flag(
                        tenant_id, recurso_id
                    )
                    target_estado = "expirado" if auto_archive else "activo"
                    await self.db.update_recurso_estado(
                        tenant_id, recurso_id, target_estado
                    )
                    logger.info(
                    ...
                    text_to_embed = f"{ext_info.get('title', '')} | {ext_info.get('summary', '')} | Tags: {keywords_str}"
                    logger.info(
                        f"[{trace_id}] [TENANT:{tenant_id}] Generando embedding. Text Length: {len(text_to_embed)}"
                    )

                    vector = await self._generate_embedding(text_to_embed, trace_id)
                    logger.info(
                        f"[{trace_id}] Generado vector de {len(vector)} dimensiones"
                    )
                    ...
                    logger.info(
                        f"[{trace_id}] Vector inyectado exitosamente en Qdrant. Aislado a tenant_id: {tenant_id}"
                    )
                ...
                if self.redis:
                    ...
                    logger.info(
                        f"[{trace_id}] Publicado evento ingest:complete (estado={estado_notif}) para tenant {tenant_id}"
                    )

            except Exception as e:
                logger.error(
                    f"[{trace_id}] F-03.2 Fallo crítico procesando embedding/qdrant: {e}"
                )
                raise e
    ...
    async def _emit_auto_archive_event(
        ...
        try:
            ...
            logger.info(
                f"[{trace_id}] Outbox event recurso.expirado/auto_archive emitido"
            )
        except Exception as e:
    ...
    async def _build_chunks_from_text(
        ...
        if not contenido.strip():
            logger.warning(
                f"[{trace_id}] Sin contenido para chunkear (recurso={recurso_id}); RAG por chunks vacío."
            )
            return
        ...
        await self._inject_chunks_to_qdrant(
            recurso_id, tenant_id, url, title, chunks, vectors, trace_id
        )
    ...
    async def _replicate_or_build_chunks(
        ...
        existing = [
            p
            for p in existing
            if (p.get("payload") or {}).get("tenant_id") != tenant_id
        ]
        if existing:
            points = []
            for p in existing:
            ...
            resp.raise_for_status()
            logger.info(
                f"[{trace_id}] {len(points)} chunks clonados de tenant origen → {tenant_id} (recurso={recurso_id})"
            )
            return
        # Sin chunks fuente (recurso ingerido antes del chunking): chunkear desde Postgres
        await self._build_chunks_from_text(
            recurso_id, tenant_id, url, title, "", trace_id
        )
