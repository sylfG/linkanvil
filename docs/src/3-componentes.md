...
- Para puertos, imágenes, límites de RAM/CPU y otros datos operativos consulta el [catálogo de contenedores](./2-resumen-servicios.md).
- Para flujos de trabajo, decisiones de persistencia, seguridad, schema isolation, migraciones y observabilidad consulta [`4-arquitectura.md`](./4-arquitectura.md).
...
- **Control de Multitudes (Rate Limiting):** para prevenir sobrecargas o ataques, bloquea automáticamente los excesos. El máximo promedio es **100 req/s por IP**, con `burst` de 50, configurado en el middleware global de Traefik (`traefik.http.middlewares.global-ratelimit`).
- **Reintentos a Prueba de Fallos:** si uno de los servicios internos falla temporalmente al intentar responder, Traefik vuelve a intentar conectarse hasta 3 veces automáticamente antes de devolver un error.
- **Etiqueta de Seguimiento (Trace-ID):** coloca un identificador único (como un gafete de visitante numerado) a cada petición. Esto permite rastrear el recorrido por todos los sistemas si necesitas diagnosticar un problema.

**¿Por qué se tomó esta decisión?**

- **Seguridad Centralizada:** al tener un único punto de entrada, las reglas pesadas (restricciones, reintentos y seguridad) se aplican una sola vez en la puerta principal. Así, no tienes que programar estas mismas defensas individualmente en cada aplicación del sistema.
- **Configuración Invisible:** Traefik lee de manera automática las labels Docker incrustadas en los demás servicios. Esto permite que el sistema sepa adónde dirigir el tráfico sin necesidad de crear ni actualizar documentos de configuración largos y propensos a errores humanos.
- **Monitoreo Transparente:** avisa a las herramientas de vigilancia (Prometheus y OpenTelemetry) qué rutas fallan y cuánto tardan en responder.
- **Seguridad Web en Producción:** cuando se despliega en internet, Traefik se encarga por sí solo de solicitar los certificados Let's Encrypt y fuerza a que todo el mundo use conexiones cifradas (convierte HTTP normal a HTTPS).
...
El bot de Telegram exige que LinkAnvil exponga `POST /webhook/telegram/{token_hash}` en una URL HTTPS pública. En entornos detrás de NAT no la hay, y el host interno `cerebro-ingestion:8000` solo es resoluble dentro de `cerebro-net`. La solución es un sidecar `tailscale/tailscale:stable` (profile `telegram`) que se conecta a la tailnet del usuario, declara un proxy hacia `cerebro-ingestion:8000` y publica al exterior con `AllowFunnel: true`.
...
- **Seguridad sin Privilegios:** opera sin pedir permisos delicados al sistema operativo.

**¿Por qué se tomó esta decisión?**

Tailscale Funnel se eligió sobre Cloudflare Tunnel/ngrok porque (a) es gratuito sin necesidad de dominio propio, (b) el subdominio se mantiene fijo entre `docker compose down/up` mientras viva el volumen `cerebro-tailscale-state`, (c) HTTPS con certificado Let's Encrypt es automático, (d) no requiere abrir puertos en el router.
...
Los pasos de configuración están en [`1-instalacion-configuracion.md`](./1-instalacion-configuracion.md), sección 6.4.
...
Es un servicio ligero dedicado exclusivamente a recibir enlaces nuevos (desde la extensión de navegador, Telegram u otras aplicaciones). Antes de poner el enlace en la cola de trabajo realiza dos filtros ultrarrápidos:

- **Filtro de Duplicados:** utiliza una estructura probabilística rápida en memoria (Bloom Filter en Redis) para comprobar en milisegundos si ese enlace ya existe en el sistema. El bloom es un *hint*, no un veto — la idempotencia real se resuelve aguas abajo.
...
> El cliente principal que dispara `POST /ingest` es la extensión de navegador del proyecto; también hay flujo Telegram (`POST /webhook/telegram/{token_hash}`).
...
- **Atomicidad del rate limit:** el control de abusos hace su comprobación en un solo paso indivisible (`INCR+EXPIRE` atómico). Esto evita la *race condition* que aparece cuando dos peticiones casi simultáneas leen el mismo contador, lo incrementan por separado y permiten más tráfico del esperado. Además, la comunicación con Redis se hace de forma asíncrona, así que el proceso no se bloquea esperando respuestas.
...
El backend se rediseñó para priorizar la seguridad, abandonando el sistema anterior que guardaba las "llaves" del usuario en lugares poco seguros. Sus defensas incluyen:

- **Cajas Fuertes Temporales (httpOnly):** guarda el token de acceso en una cookie que el navegador web puede usar pero no puede leer directamente. Evita que un XSS robe credenciales.
...
- **Pool HTTP Compartido:** en lugar de abrir y cerrar una conexión HTTP nueva cada vez que consulta a LiteLLM, reutiliza un cliente HTTP asíncrono compartido. Esto ahorra memoria y evita colapso bajo presión.

### 🌐 cerebro-web — Frontend Next.js

*Analogía:* Imagina el tablero de mandos de un coche moderno. Es la pantalla táctil que tocas para interactuar con el vehículo. Además, la pantalla guarda tu música favorita en una caja fuerte central protegida, no en una memoria frágil detrás de la pantalla que cualquiera podría sacar.
...
- **Mayor Seguridad y Control:** se reemplazó la tecnología anterior (Streamlit) porque era demasiado rígida. No permitía gestionar inicios de sesión complejos ni manejar cookies httpOnly. El frontend Streamlit se conserva como referencia histórica pero no está cableado en el stack; el frontend operativo es exclusivamente Next.js.
...
- **Solución a "Mensajes Fantasma":** antes, los chats se guardaban en `localStorage`. Esto provocaba que, incluso si borrabas el sistema por completo, al volver a entrar aparecieran mensajes viejos. Ahora todo se pide directamente a la base de datos oficial mediante un store API-backed.
...
A veces, las páginas web detectan que somos un robot e intentan bloquearnos mostrando páginas de error (ej. "¡Verifica que no eres un robot!"). El scraper reconoce esos mensajes mediante una heurística específica (marcadores explícitos para evitar falsos positivos como `cdnjs.cloudflare.com`) más un guard de longitud mínima de 300 caracteres. Si esto ocurre, en lugar de guardar texto basura, mueve el recurso al estado `cuarentena` (sin DLQ) para revisión manual.

**¿Por qué se tomó esta decisión?**

Separar la lectura de textos en un servicio independiente permite que la aplicación principal siga funcionando aunque estemos descargando cientos de páginas web muy pesadas. Además, a este trabajador se le asigna mucha más RAM (con `shm_size: 1gb` para Chromium) para que su navegador invisible no colapse bajo presión.
...
- **Reciclaje Inteligente:** puesto que las matemáticas de un texto público siempre son las mismas sin importar quién lo guarde, si un usuario guarda un enlace que otro usuario ya había procesado, el embedder simplemente reutiliza los vectores antiguos (flujo `reused=true`).
...
- **Notificación Telegram:** si el tenant tiene bot configurado y un `chat_id` capturado (cacheado en Redis bajo `telegram_chat:{tenant_id}` y persistido en `usuarios.telegram_chat_id`), envía un mensaje directo al chat.
...
- **Interruptor de Seguridad (Circuit Breaker):** si un proveedor falla `allowed_fails: 1` vez, deja de enviarle peticiones durante `cooldown_time: 15` segundos.
- **Memoria de Ahorro (Caché):** recuerda las respuestas de `completion` y `embedding` en Redis con TTL de 1 hora (`cache_params.ttl: 3600`).
- **Persistencia de Modelos:** mantiene el catálogo de modelos en Postgres (`store_model_in_db: true`) — por eso depende de la BD principal aunque Prisma limpie el schema `public`.

**¿Por qué se tomó esta decisión?**

Nos da libertad absoluta. Si mañana queremos cambiar de proveedor de IA, solo tenemos que cambiar un par de líneas en la configuración de LiteLLM, sin tener que reprogramar el código principal del sistema.

**Nota de Seguridad:** LiteLLM (vía Prisma) limpia la zona `public` de la base de datos cada vez que se enciende. Por eso todas nuestras tablas se construyeron en un schema separado y exclusivo llamado `cerebro` — ver el detalle en [`4-arquitectura.md`](./4-arquitectura.md), sección de Aislamiento de Schema.
...
- Se encarga de tareas de coordinación repetitivas como escuchar webhooks, conectar la plataforma con bots externos, o programar limpiezas nocturnas de datos (ver el flujo de Curación Nocturna en [`4-arquitectura.md`](./4-arquitectura.md)).
...
- Espera a que `cerebro-n8n` esté en su puesto.
...
- **Privacidad Estricta:** usa Row-Level Security para asegurar que un usuario solo pueda ver sus propios enlaces guardados (vía la tabla `usuario_recursos`), aunque el sistema recicle y comparta los textos "tras bambalinas".
...
Es un contenedor `postgres:16-alpine` con `restart: "no"` que se ejecuta una sola vez al levantar el stack. Aplica las migraciones SQL pendientes versionadas (numeradas `0001_…` a `00NN_…`) contra la base de datos. Es idempotente: lleva registro en `cerebro.schema_migrations(version)` para no re-aplicar lo ya aplicado. `cerebro-api` depende de su `service_completed_successfully`, así que la API no arranca si las migraciones fallan.
...
3. **Monitor de Vida (Heartbeat):** los workers dejan aquí su firma cada 15 segundos (con TTL de 45 s) para avisar al sistema que siguen vivos (ver Heartbeat en [`4-arquitectura.md`](./4-arquitectura.md)).
...
Aunque Postgres puede guardar vectores (con `pgvector`), Qdrant fue diseñado *únicamente* para buscar similitudes semánticas. Es cien veces más rápido (gracias a los índices HNSW) y permite filtrar por `tenant_id` sin necesidad de JOINs adicionales.
...
Nos da flexibilidad a futuro. Si mañana decidimos cambiar la pantalla donde analizamos los errores (por ejemplo, dejar de usar Jaeger), solo le avisamos al recolector, sin tener que reprogramar las aplicaciones.
...
Recopila métricas de salud de todo el sistema de manera periódica. Cuenta con reglas automáticas (alertas) configuradas — ver el detalle en la sección de Observabilidad de [`4-arquitectura.md`](./4-arquitectura.md).
...
Reúne toda la información dura del supervisor estadístico (Prometheus) y el detective de tiempos (Jaeger) mostrándolas en un mismo panel fácil de leer.
...
> Continúa con los flujos de trabajo y decisiones arquitectónicas en [`4-arquitectura.md`](./4-arquitectura.md).
